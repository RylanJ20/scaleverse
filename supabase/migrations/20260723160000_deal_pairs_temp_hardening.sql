-- Hardening: drop a leftover _deal_pairs temp table before creating it.
-- `on commit drop` covers the one-call-per-transaction PostgREST path, but any
-- context that runs deal_matchups twice in one transaction (tests, scripts,
-- future batch jobs) hit 42P07 on the second call. Function body otherwise
-- identical to 20260723154500.
...
begin
  ...
  if v_remainder > 0 then
    ...
    if v_pending < 40 and v_dealt_hour < 400 then
      drop table if exists _deal_pairs;
